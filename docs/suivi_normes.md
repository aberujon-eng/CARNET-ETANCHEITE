...
| N5 | Lecture C5F1 protections mécaniques → extraits (chapes/écrans : folios 15/33/34/36/40-43/46-48/51 « béton de protection », écran sup DEG) | **FAIT** (2026-07-14 : protection complémentaire **6 cm** → tranche la question « protection lourde 5 cm » (à appliquer N11) ; écran PVC 1,90 mm mini corrobore C1F1 → info pour point BLOQUÉ 2, sans action ; géotextiles 700/1000/1200 g/m² — `extraits_C5F1.md`) |
| N6 | Lecture C9F1 mixte + raccordements existants → extraits (folios 45, 61-63 raccords DEG/FPM/SELA ; réparations) | **FAIT** (2026-07-14 : 2 solutions types raccord DEG/FPM = info pour point BLOQUÉ 4 sans action ; §6 voussoirs/station = base folio « arrêt DEG voussoir » ; retour SEL-A 50 cm intrados — `extraits_C9F1.md`) |
| N7 | Lecture **C10F1 structures intégrées (publication)** → extraits (pertinence folios gares/structures : 12-25, 61-64) | **FAIT** (2026-07-14, lu intégralement : seuil 8 m (info point BLOQUÉ 5) ; règle R3/R4 selon hauteur d'eau ; §7 = base folios 30-33 ; §8.6 valide folios 34/36 ; §9 dalle couverture (retombée ≥20 cm, Bazin-Chéry) ; §11 = base folios 40-43 — `extraits_C10F1.md`) |
| N8 | Lecture STRRES FAEQ2 ch. 3/4/5 (diagnostic, réparation, essais) → extraits (folio réparation DEG à créer ; NOTA contrôles) | **FAIT** (2026-07-14 : chapitres tunnels = renvois aux recommandations AFTES déjà lues ; définitions DEG-P (extrusion/calandrage, 2 mm, soudures 30/2×12) — `extraits_STRRES.md`) — **LECTURE DU CORPUS TERMINÉE** |
...
| N10 | Relevé du guide couleur du carnet (légendes DXF, calques) → `docs/normes/guide_couleur.md` | **FAIT** (2026-07-14 : système de calques sémantiques SYSTEME_*/Matériel_*/Produit_* documenté avec ACI + règles d'intervention graphique BYLAYER) |
| N11 | Application reprises TEXTE (NOTA, renvois normatifs) issues de N1-N9 — hors 5 points bloqués | **FAIT** (2026-07-14 : 34× « protection lourde 5cm » → **6 cm** [C5F1+P34/36+P47] ; NOTA F30 pipettes/boîtiers [C10F1 §7.1] ; NOTA F32 surfaces compartimentage [F67-III 3.3.1] ; NOTA F46 dalle couverture [C10F1 §9] ; 0 résidu, caches purgés — `scripts/apply_normes_n11.py`, `docs/rapport_n11.txt`) |
